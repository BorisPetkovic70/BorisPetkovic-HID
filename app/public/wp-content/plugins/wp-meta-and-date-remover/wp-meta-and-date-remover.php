...
if ( function_exists( 'wmadr_fs' ) ) {
    wmadr_fs()->set_basename( false, __FILE__ );
} else {
    ...
    function wpmdr_dashboard_scripts()
    {
        if ( get_option( 'wpmdr_yoast_datepublished', 0 ) == 1 ) {
            wp_enqueue_style( 'wpmdr_yoast_css', plugin_dir_url( __FILE__ ) . 'admin/css/yoast.css' );
        }
        wp_enqueue_script( 'wpmdr_yoast_js', plugin_dir_url( __FILE__ ) . 'admin/js/yoast.js', [ 'jquery' ] );
        wp_localize_script( 'wpmdr_yoast_js', 'obj', [
            'upgrade_url'      => wmadr_fs()->get_upgrade_url(),
            'is_paying'        => !wmadr_fs()->is_not_paying(),
            'is_yoast_enabled' => get_option( 'wpmdr_yoast_datepublished', 0 ) == 1,
        ] );
    }
    ...
    function wpmdr_hide_yoast_snippet()
    {
        //[class='SnippetPreview__DatePreview']{ display:none!important }
    }
    ...
    add_action( 'admin_init', 'wpmdr_dashboard_scripts' );
}
